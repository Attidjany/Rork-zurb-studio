...
RSYNC="sshpass -p $SSH_PASS rsync -az --delete -e 'ssh -o StrictHostKeyChecking=no'"
...
eval $RSYNC --exclude proxy.php --exclude .htaccess "$ROOT/expo/dist/" "$SSH_USER@$SSH_HOST:$PUBLIC/"
eval sshpass -p $SSH_PASS rsync -az -e "'ssh -o StrictHostKeyChecking=no'" "$ROOT/deploy/public_html/" "$SSH_USER@$SSH_HOST:$PUBLIC/"

echo "▶ uploading api → $API_DIR"
$SSH "mkdir -p $API_DIR/dist"
eval sshpass -p $SSH_PASS rsync -az -e "'ssh -o StrictHostKeyChecking=no'" "$ROOT/server/dist/index.js" "$SSH_USER@$SSH_HOST:$API_DIR/dist/index.js"
$SSH "cd $API_DIR && [ -f package.json ] || echo '{\"name\":\"zurb-api\",\"type\":\"module\",\"private\":true}' > package.json"

if [ $SETUP = 1 ]; then
  echo "▶ installing .env + pm2"
  eval sshpass -p $SSH_PASS rsync -az -e "'ssh -o StrictHostKeyChecking=no'" "$ROOT/server/.env.production" "$SSH_USER@$SSH_HOST:$API_DIR/.env"
