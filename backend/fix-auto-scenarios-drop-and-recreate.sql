-- Fix: Drop function first before recreating with new return type
-- This resolves: ERROR: 42P13: cannot change return type of existing function

-- 1. Drop ALL existing auto-scenario related functions and triggers
...
DROP FUNCTION IF EXISTS check_site_blocks_configured(uuid) CASCADE;

-- 2. Recreate generate_auto_scenarios with proper return type and cleanup
...
-- 3. Add indexes to improve performance (only if they don't exist)
