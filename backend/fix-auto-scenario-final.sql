-- Final fix for auto-scenario duplicate key issue
-- The problem is that DELETE FROM scenarios should cascade delete related records,
-- but we need to ensure proper cleanup

-- Drop and recreate the function with proper cleanup
...
BEGIN
  ...
  -- Get IDs of existing auto-scenarios for this site
  SELECT ARRAY_AGG(id) INTO v_scenario_ids
  FROM scenarios
  WHERE site_id = p_site_id
    AND is_auto_scenario = TRUE;
  
  -- Explicitly delete all related records first
  IF v_scenario_ids IS NOT NULL THEN
    -- Delete from child tables first
    DELETE FROM scenario_construction_costs
    WHERE scenario_id = ANY(v_scenario_ids);
    ...
    -- Then delete the scenarios themselves
    DELETE FROM scenarios
    WHERE id = ANY(v_scenario_ids);
  END IF;
